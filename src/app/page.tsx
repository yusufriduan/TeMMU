      ...
      <TabGroup className={"flex flex-col items-center mt-10 gap-10"}>
        <div className='flex flex-row justify-between items-center w-[90vw]'>
          ...
          <div className='flex flex-row content-between items-center gap-6'>
            <TabList className={"flex gap-10 bg-(--foreground) rounded-4xl w-auto p-4"}>
              <Tab className={"rounded-4xl p-2 data-hover:bg-(--hover) hover:cursor-pointer data-selected:bg-(--highlighted) data-selected:text-white data-seleceted:font-bold transition-[background-color,color] duration-300 ease-in-out"}>
                Workspace
              </Tab>
              <div className={"flex"}> {/* Roles Exclusives */}
                <Tab className={" flex rounded-4xl p-2 data-hover:bg-(--hover) hover:cursor-pointer data-selected:bg-(--highlighted) data-selected:text-white data-seleceted:font-bold transition-[background-color,color] duration-300 ease-in-out"}>
                  Find Mentors
                </Tab>
          ...
          <div className='flex justify-end w-1/8'>
            <Menu>
              ...
              <MenuItems anchor="bottom end" transition className="bg-(--foreground) rounded-3xl p-2 flex flex-col gap-2 border-black border w-(--button-width) --anchor-gap: 8px origin-top transition duration-200 ease-out data-closed:scale-95 data-closed:opacity-0">
                <MenuItem>
                  <a className="block pl-2 data-focus:bg-(--hover) rounded-2xl p-1 transition duration-300 ease-in-out" href="/profile">
                    My Profile
                  </a>
                </MenuItem>
                <MenuItem>
                  <a className="block pl-2 data-focus:bg-(--hover) rounded-2xl p-1 transition duration-300 ease-in-out" href="/logout">
                    Logout
                  </a>
                </MenuItem>
              </MenuItems>
        ...
        <TabPanels>
          <TabPanel className={"w-[90vw] h-[80vh] bg-(--foreground) rounded-4xl overflow-y-auto"}>Workspaces</TabPanel>

          <TabPanel className={"flex w-[90vw] h-[80vh] bg-(--foreground) rounded-4xl overflow-y-clip"}> {/* Find Mentors Container */}
            <div className='flex flex-col justify-center items-center w-full h-full'>
              ...
              <form className='flex flex-row gap-4'>
                <Input name='full_name' type='text' className={'bg-blue-400 rounded-lg p-2 text-black'} placeholder='Type the name!'></Input>
                <Button className={'bg-blue-400 rounded-lg p-2 text-black data-hover:bg-(--highlighted) data-hover:cursor-pointer data-hover:text-white transition duration-300 ease-in-out'}>Search</Button>
              </form>
          ...
          <TabPanel className={"w-[90vw] h-[80vh] bg-(--foreground) rounded-4xl"}> {/* Manage Mentees Container */}
            <div className='flex flex-col justify-center items-center w-full h-full overflow-y-auto'>
              ...
              <div className='flex flex-col gap-4 w-[80%] h-[60%] overflow-y-auto'>
                ...
                <div className='flex flex-row justify-between items-center p-4 bg-blue-300 rounded-lg shadow-lg'>
                  ...
                  <Button className='bg-red-500 text-white px-4 py-2 rounded-lg hover:cursor-pointer hover:font-bold'>Remove Mentee</Button>
                </div>
                <div className='flex flex-row justify-between items-center p-4 bg-blue-300 rounded-lg shadow-lg'>
                ...
                <div className='flex flex-col gap-4 w-full h-[40%]'>
                  ...
                  <div className='flex flex-row justify-between items-center p-4 bg-blue-300 rounded-lg shadow-lg'>
                    ...
                    <div className='flex flex-row gap-2'>
                      <Button className='bg-green-500 text-white px-4 py-2 rounded-lg hover:cursor-pointer hover:font-bold'>Accept</Button>
                      <Button className='bg-red-500 text-white px-4 py-2 rounded-lg hover:cursor-pointer hover:font-bold'>Decline</Button>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <TabPanel className={"w-[90vw] h-[80vh] bg-(--foreground) rounded-4xl"}>
            <div className='flex flex-row justify-between items-center h-full'> {/* Chats container */}
              <div className='flex flex-col justify-center align-center w-[45vw] h-full'> {/* Chat header would go here */}
                <div className='flex flex-col flex-wrap gap-2 p-4 h-full'>
                  ...
                  <div className='flex flex-col align-top gap-4'>
                    ...
                    <div className='flex flex-row justify-between items-center p-2 bg-(--bg-section) rounded-lg shadow-lg hover:cursor-pointer hover:font-bold hover:bg-(--hover) data-selected:bg-(--highlighted) transition duration-200 ease-in-out'>
                      ...
                      <span className='text-sm text-gray-600'>2 new messages</span>
                    </div>
                    <div className='flex flex-row justify-between items-center p-2 bg-(--bg-section) rounded-lg shadow-lg hover:cursor-pointer hover:font-bold hover:bg-(--hover) data-selected:bg-(--highlighted) transition duration-200 ease-in-out'>
                      <span>Chat with Mentor 2</span>
                      <span className='text-sm text-gray-600'>No new messages</span>
                    </div>
                  </div>
                  ...
                  <div className='hidden flex-col items-center justify-center gap-4 mt-10'>
                    ...
                    <Button className='bg-blue-400 border border-brand-subtle text-fg-brand-strong px-4 py-2 rounded-lg hover:cursor-pointer hover:font-bold hover:bg-(--highlighted) hover:scale-120 transition duration-200 ease-in-out'>Start New Chat</Button>
                  </div>
                </div>
              </div>
              ...
              <div className='flex flex-col items-center justify-center flex-wrap gap-2 p-4 w-[45vw] h-full overflow-y-auto'> {/* Chat window would go here */}
                <div className='flex flex-col items-center justify-center gap-4 mb-10 h-full'> {/* No chat selected */}
                  <p>Select a chat to start messaging.</p>
                </div>
                <div className='hidden flex-col h-full'> {/* Chat selected */}
          ...
          <TabPanel className={"w-[90vw] h-[80vh] bg-(--foreground) rounded-4xl"}>
            <div className='flex flex-col w-full h-full pt-4 pl-4'> {/* Forums container */}
